...
export const BusinessAnalysisPage: React.FC = () => {
  ...
  if (isLoading) {
    return <LoadingScreen message="Synthesizing requirements, AS-IS process, and stakeholder analysis..." />;
  }
    ...
    <div className="space-y-8 animate-fadeIn max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-800">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <h1 className="text-2xl font-extrabold text-white">Business Analysis & Current State</h1>
          </div>
          <p className="text-xs text-slate-400 mt-1">
            Requirement discovery, AS-IS process bottlenecks, stakeholder alignment, and operational KPIs.
          </p>
        </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          >
            ...
            <span>{isRegenerating ? 'Analyzing...' : 'Regenerate Analysis'}</span>
          </button>
          <Link
          ...
          >
            <span>Next: 8-Dimension Gaps</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="p-6 rounded-2xl bg-slate-900/60 border border-slate-800 backdrop-blur-md flex flex-col justify-between">
          ...
          <div className="mt-4 pt-3 border-t border-slate-800 text-[11px] text-slate-400">
            Confidence Index: <span className="text-emerald-400 font-bold">95.4%</span> (Grounded in context)
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-sm font-bold text-slate-100 flex items-center">
              ...
              Current State (AS-IS) Manual Workflow & Bottlenecks
            </h3>
            <p className="text-xs text-slate-400">Step-by-step audit of operational friction points before transformation.</p>
          </div>
          ...
          {data?.as_is_process.map((step) => (
            ...
            >
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h4 className="font-bold text-slate-100">{step.activity}</h4>
                  <p className="text-[11px] text-slate-400">Actor: <span className="text-slate-300 font-medium">{step.actor}</span> | System: <span className="text-slate-300 font-medium">{step.system}</span></p>
                </div>
              </div>
      ...
      <div className="p-6 rounded-2xl bg-slate-900/60 border border-slate-800 backdrop-blur-md">
        <h3 className="text-sm font-bold text-slate-100 flex items-center mb-4">
          ...
          Discovered Functional & Non-Functional Requirements
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {data?.functional_requirements.concat(data?.non_functional_requirements || []).map((req) => (
            <div key={req.code} className="p-4 rounded-xl bg-slate-800/50 border border-slate-700/60 text-xs">
              ...
              <h4 className="font-bold text-slate-100 mb-1">{req.title}</h4>
              <p className="text-slate-400 leading-relaxed mb-2">{req.description}</p>
              <span className="text-[10px] text-slate-500 block">Source: {req.source || 'Document Analysis'}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="p-6 rounded-2xl bg-slate-900/60 border border-slate-800 backdrop-blur-md">
        <h3 className="text-sm font-bold text-slate-100 flex items-center mb-4">
          ...
          Stakeholder Matrix & Impact Analysis
        </h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {data?.stakeholders.map((sh, idx) => (
            <div key={idx} className="p-4 rounded-xl bg-slate-800/40 border border-slate-700/60 text-xs">
              <h4 className="font-bold text-slate-100">{sh.name}</h4>
              <p className="text-[11px] text-blue-400 font-medium">{sh.role}</p>
              <p className="text-[11px] text-slate-400">{sh.department}</p>
              <div className="mt-3 pt-2 border-t border-slate-700 flex items-center justify-between text-[10px]">
                <span className="text-slate-500">Influence: <b className="text-slate-300">{sh.influence}</b></span>
                <span className="text-slate-500">Interest: <b className="text-slate-300">{sh.interest}</b></span>
              </div>
              <p className="text-[11px] text-slate-400 mt-2 italic line-clamp-2">"{sh.key_concerns}"</p>
            </div>
          ))}
        </div>
      </div>
